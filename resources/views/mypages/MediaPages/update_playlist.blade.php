...
<div class="adding_playlist_container main-container animated slideInUp">
    ...
    <div class="top-title text-center mt-3">Update Playlist</div>
    ...
    <div class="form-container w-100 mt-2 mb-2 disable-scrollbars">
        <form action="{{ route('update_playlist') }}" method="POST" class="w-100 flex-form" id="add-playlist-form">
            @csrf
            <input type="text" name="playlist_id" style="display: none" value="{{ $playlist->id }}">
            <div class="form-group w-75">
                <div class="text-field w-100">
                    <input type="text" name="playlist_title" placeholder="Playlist's title" tabindex="1" value="{{ $playlist->name }}" required>
                </div>
            </div>
            <div class="form-group w-75">
                <div class="text-field w-100">
                    <input type="text" name="playlist_description" placeholder="Description" tabindex="2" value="{{ $playlist->description }}">
                </div>
            </div>
            ...
            <div class="form-group w-75 d-flex justify-content-center">
                <button type="submit" name="submit" class="form-submit-btn" id="upload-btn">Update</button>
            </div>
        </form>
    </div>
</div>
...
<script src="{{ URL::asset('js/mypages/Medias/update_playlist_page.js') }}"></script>
@endsection